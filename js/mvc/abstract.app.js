define([], function () {
  ...
  return _.inherit({
    ...
    createViewPort: function () {
      ...
      this.container.on('onselectstart',function(){return false});
    ...
    switchView: function (path) {
      this.container.removeClass('overflow-h');
      this.container.find('#main').removeClass('overflow-h');
      var id = path;
    ...
    getViewIdRule: function (url) {
      ...
      if (this.hasPushState) {
        ...
        viewId = url.replace(/^[^#]+(#(.+))?/g, '$2').replace(/^#+/i, '');//.toLowerCase()
        h = /^([^?&|]*)(.*)?$/i.exec(viewId);
        vp = h[1] ? h[1].split('!') : [];
